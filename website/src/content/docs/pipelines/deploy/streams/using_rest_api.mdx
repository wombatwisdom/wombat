...
By using the Wombat `streams` mode REST API you can dynamically control which streams are active at runtime. The full
spec for the Wombat streams mode REST API can be [found here](/reference/streams_api).

Note that stream configs created and updated using this API do *not* benefit from
[environment variable interpolation](/pipelines/learn/interpolation) (function interpolation will still work).
