...
When running Wombat in `streams` mode it's possible to create streams with their own static configurations, simply
list one or more files after the `streams` subcommand:
...
## Resources
A stream configuration should only include the base stream component fields (`input`, `buffer`, `pipeline`, `output`),
and therefore should NOT include any [resources](/pipelines/learn/resources). Instead, define resources separately and import them
using the `-r`/`--resources` flag:
...
There are other endpoints [in the REST API](../using_rest_api) for creating, updating and deleting streams.
