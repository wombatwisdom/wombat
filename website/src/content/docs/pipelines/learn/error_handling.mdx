...
Wombat supports a range of processors such as `http` and `aws_lambda` that have the potential to fail if
...
If this behaviour is not desired then it is possible to simulate a catch block with a `switch` processor:
...
rejection) on data that has failed to process rather than delivering it to our output with a `reject_errored` output:
...
And by placing the above within a `fallback` output we can instead route the failed messages to a
different output:
