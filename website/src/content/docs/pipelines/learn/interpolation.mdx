...
range of [functions](/bloblang/reference/functions). For example, with the following config:
...
### Examples
<Tabs>
    <TabItem label="Reference Metadata">
    ...
    <TabItem label="Coalesce and Mapping">
    ...
    <TabItem label="Delayed Processing">
    ...
    <TabItem label="Reusing Configuration Snippets">
        Sometimes it's necessary to use a rather large component multiple times. Instead of copy/pasting the configuration or
        using YAML anchors you can define your component [as a resource](/pipelines/learn/resources).
