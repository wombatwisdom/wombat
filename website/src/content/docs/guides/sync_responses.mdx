...
It's possible to route the result of any Wombat processing pipeline directly back to an input with a [`sync_response`][sync-res] output:
...
With the above example a message received from the endpoint `/post` would be sent unchanged to the address `http://localhost:4196/post`, and then the response from that request would get returned back. This basically turns Wombat into a proxy server with the potential to mutate payloads between requests.
