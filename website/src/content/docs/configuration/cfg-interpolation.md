...
syntax `${<variable-name>}` (or `${<variable-name>:<default-value>}` in order to specify a default value). This is
useful for setting environment specific fields such as addresses:
...
range of [functions][bloblang_functions]. For example, with the following config:
