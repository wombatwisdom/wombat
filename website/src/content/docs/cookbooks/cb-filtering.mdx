...
the [`mapping` processor][processors.mapping] but shouldn't require any prior knowledge.

## The Basic Filter

Dropping events with [Bloblang][guides.bloblang] is done by mapping the function `deleted()` to the `root` of the 
...
[`match`][bloblang.match] or [`if`][bloblang.if] expression:
