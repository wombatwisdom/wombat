...
input:
  kafka:
    addresses: [ TODO ]
    topics: [ comments ]
    consumer_group: wombat_comments_group
...
There are [many patterns for error handling][error-handling] to choose from in Wombat. In this example we're going to introduce a delayed retry queue as it enables us to reprocess failed documents after a grace period, which is isolated from our main pipeline.
