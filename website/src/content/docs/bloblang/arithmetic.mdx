...
a [recoverable mapping error will be thrown](/pipelines/learn/error_handling).
...
as an argument then a [recoverable mapping error will be thrown](/pipelines/learn/error_handling).

### Boolean

Boolean comparison operators (`||`, `&&`) are valid to use against boolean values only (`true` or `false`). If a
non-boolean value is used as an argument then a [recoverable mapping error will be thrown](/pipelines/learn/error_handling).