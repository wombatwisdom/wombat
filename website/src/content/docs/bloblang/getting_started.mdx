...
Bloblang supports a bunch of [literal types](/bloblang/#literals), and the first line of this mapping assigns an empty object
...
which [there are many](/bloblang#methods). To demonstrate we're going to change our mapping
to [uppercase](/reference/bloblang/methods/#uppercase) the field `message` from our input document:
...
`replace_all` method](/reference/bloblang/methods/#replace_all):
...
a target, and there are [plenty of them as well](/reference/bloblang/functions). Functions are often used to extract information
unrelated to the input document, such as [environment variables](/reference/bloblang/functions/#env), or to generate data such
as [timestamps](/reference/bloblang/functions/#now) or [UUIDs](/reference/bloblang/functions/#uuid_v4).
...
`deleted()`](/reference/bloblang/functions/#deleted). To illustrate let's create a field we want to delete by changing our input to the
following:
...
An alternative way to delete fields is the [method `without`](/reference/bloblang/methods/#without), our above example could be
...
be structured, it is possible to reference it as a binary string with the [function `content`](/reference/bloblang/functions/#content),
...
the [error handling guide](/pipelines/learn/error_handling), but the first step is to create the error. Luckily, Bloblang
has a range of ways of creating errors under certain circumstances, which can be used in order to validate the data
being mapped.

There are [a few helper methods](/reference/bloblang/methods/#coercion) that make validating and coercing fields nice and easy, try
this mapping out:
...
`throw` function](/reference/bloblang/functions/#throw), which takes an argument string that describes the error. When it's called it
will throw a mapping error that abandons the mapping (unless it's caught, psych!)

For example, we can check the type of a field with the [method `type`](/reference/bloblang/methods/#type), and then throw an error if
...
for [manipulating structured data types](/reference/bloblang/methods/#object--array-manipulation), let's take a quick tour of some of
...
them to share with our friends. We can do this with a [`filter` method](/reference/bloblang/methods/#filter):
...
Another such method is [`map_each`](/reference/bloblang/methods/#map_each), which allows you to mutate each element of an array, or
...
the [methods page](/reference/bloblang/methods/#object--array-manipulation).
...
Named maps are executed with the [method `apply`](/reference/bloblang/methods/#apply), which has a string parameter identifying the map
...
own [unit testing capabilities](/pipelines/test) that you can also use for your mappings. To start with save
...
in [this document](/pipelines/test).

## Final Words

That's it for this walkthrough, if you're hungry for more then I suggest you re-evaluate your priorities in life. If you
have feedback then please get in touch, despite being terrible people the Wombat community are very
welcoming.
