...
This processor uses [GoAWK](https://github.com/benhoyt/goawk), in order to understand the differences in how the program works you can read more about it in [goawk.differences](https://github.com/benhoyt/goawk#differences-from-awk).

## Fields

### `codec`

A codec defines how messages should be inserted into the AWK program as variables. The codec does not change which custom Redpanda Connect functions are available. The `text` codec is the closest to a typical AWK use case.
...
<Tabs>
<TabItem label="JSON Mapping and Arithmetic">
...
<TabItem label="Stuff With Arrays">
...
dot separated path and returns it as a string.
...
dot separated path, the value argument will be interpreted
as a string.
...
dot separated path. If the target does not
...
message payload identified by a dot separated path.
