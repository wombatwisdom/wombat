...
A [cache resource](https://docs.redpanda.com/redpanda-connect/components/caches/about) to use for storing the current latest cursor that has been successfully delivered, this allows Redpanda Connect to continue from that cursor upon restart, rather than consume the entire state of the table.
