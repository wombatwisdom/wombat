...
Both the `id` and `index` fields can be dynamically set using function interpolations described in Bloblang queries. When sending batched messages these interpolations are performed per message part.
...
<Tabs>
<TabItem label="Elastic Cloud Serverless">
This is an example of writing data to [Elastic Cloud serverless](https://www.elastic.co/docs/current/serverless).
