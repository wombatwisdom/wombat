...
| `body_key` | `string` | `"body"` | The field key to extract the raw message from. All other keys will be stored in the message as metadata. |
| `streams` | `array` | `` | A list of streams to consume from. |
...
| `create_streams` | `bool` | `true` | Create subscribed streams if they do not exist (MKSTREAM option). |
| `start_from_oldest` | `bool` | `true` | If an offset is not found for a stream, determines whether to consume from the oldest available offset, otherwise messages are consumed from the latest offset. |
| `commit_period` | `string` | `"1s"` | The period of time between each commit of the current offset. Offsets are always committed during shutdown. |
| `timeout` | `string` | `"1s"` | The length of time to poll for new messages before reattempting. |
