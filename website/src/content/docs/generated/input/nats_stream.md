...
| `cluster_id` | `string` | `` | The ID of the cluster to consume from. |
...
| `start_from_oldest` | `bool` | `true` | If a position is not found for a queue, determines whether to consume from the oldest available message, otherwise messages are consumed from the latest. |
| `max_inflight` | `int` | `1024` | The maximum number of unprocessed messages to fetch at a given time. |
| `ack_wait` | `string` | `"30s"` | An optional duration to specify at which a message that is yet to be acked will be automatically retried. |
