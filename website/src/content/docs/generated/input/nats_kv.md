...
| `ignore_deletes` | `bool` | `false` | Do not send delete markers as messages. |
| `include_history` | `bool` | `false` | Include all the history per key, not just the last one. |
| `meta_only` | `bool` | `false` | Retrieve only the metadata of the entry |
