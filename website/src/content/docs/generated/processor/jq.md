...
|:-------|:------|:---------|:-------------|
| `query` | `string` | `` | The jq query to filter and transform messages with. |
| `raw` | `bool` | `false` | Whether to process the input as a raw string instead of as JSON. |
| `output_raw` | `bool` | `false` | Whether to output raw text (unquoted) instead of JSON strings when the emitted values are string types. |
