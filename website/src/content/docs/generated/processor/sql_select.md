...
| `where` | `string` | `` | An optional where clause to add. Placeholder arguments are populated with the `args_mapping` field. Placeholders should always be question marks, and will automatically be converted to dollar syntax when the postgres or clickhouse drivers are used. |
...
| `prefix` | `string` | `` | An optional prefix to prepend to the query (before SELECT). |
