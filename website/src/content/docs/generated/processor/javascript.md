...
|:-------|:------|:---------|:-------------|
| `code` | `string` | `` | An inline JavaScript program to run. One of `code` or `file` must be defined. |
| `file` | `string` | `` | A file containing a JavaScript program to run. One of `code` or `file` must be defined. |
| `global_folders` | `array` | `[]` | List of folders that will be used to load modules from if the requested JS module is not found elsewhere. |
